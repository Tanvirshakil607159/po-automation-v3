...
def create_export_pdf(
    ...
    buffer = BytesIO()
    bottom_m = 35 * mm if export_type == "invoice" else 15 * mm
    ...
    def _draw_page(canvas_obj, doc_obj):
        ...
        if export_type != "invoice":
            # Draw line for the footer
            canvas_obj.setStrokeColor(GRAY_LINE)
            canvas_obj.setLineWidth(0.25)
            canvas_obj.line(10 * mm, 12 * mm, A4[0] - 10 * mm, 12 * mm)
            ...
            footer_p.wrapOn(canvas_obj, available, 10 * mm)
            footer_p.drawOn(canvas_obj, 10 * mm, 8 * mm)

        if export_type == "invoice":
            left_footer_style = ParagraphStyle(
                "InvoiceFooterLeft", parent=styles["Normal"],
                fontSize=8, leading=11, alignment=TA_LEFT, textColor=BLACK,
            )
            right_footer_style = ParagraphStyle(
                "InvoiceFooterRight", parent=styles["Normal"],
                fontSize=8, leading=11, alignment=TA_RIGHT, textColor=BLACK,
            )
            left_text = (
                '<font color="#16a34a"><b>OFFICE &amp; FACTORY</b></font><br/>'
                "356/1, Block-B Tek Kathora<br/>"
                "Salna, Gazipur-1703<br/>"
                "Bangladesh"
            )
            right_text = (
                '<font color="#16a34a"><b>CONTACT DETAILS</b></font><br/>'
                "Cell: +8801766671724<br/>"
                "E-mail: maksudakumu@kadesignaccessoriesltd.com<br/>"
                "Web: www.kadesignaccessoriesltd.com"
            )
            
            footer_table_data = [[
                Paragraph(left_text, left_footer_style),
                Paragraph(right_text, right_footer_style)
            ]]
            footer_tbl = Table(footer_table_data, colWidths=[available * 0.5, available * 0.5])
            ...
            w, h = footer_tbl.wrapOn(canvas_obj, available, 25 * mm)
            footer_tbl.drawOn(canvas_obj, 10 * mm, 5 * mm)
    ...
    if export_type == "invoice" and invoice_info:
        ...
        if invoice_info.get("invoice_type") != "pi":
