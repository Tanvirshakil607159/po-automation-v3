...
export default function ExportButton() {
    ...
    return (
        <button onClick={handleExport} disabled={exporting}
            className="group flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-semibold transition-all
                bg-[#5b7a6a] text-white shadow-md hover:bg-[#4d6a5c] hover:shadow-lg
                active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed"
            id="export-btn"
        >
            {exporting ? (
                <>
                    <div className="w-3.5 h-3.5 rounded-full animate-spin border-[1.5px] border-white/30 border-t-white" />
