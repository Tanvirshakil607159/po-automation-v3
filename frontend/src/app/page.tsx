  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      <main className="flex-1 flex flex-col overflow-hidden w-full">
        {/* Top Bar */}
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 bg-white">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            {hasData && (
              <>
                ...
                <button
                  onClick={clearUpload}
                  className="hidden sm:block px-4 py-2.5 rounded-xl text-sm font-medium bg-gray-100 text-gray-500
                    hover:bg-gray-200 hover:text-gray-700 transition-all border border-gray-200"
                  id="new-upload-btn"
                >
                  New Upload
                </button>
                <button
                  onClick={clearUpload}
                  className="sm:hidden p-2 rounded-xl bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-700 transition-all border border-gray-200"
          ...
          {!hasData ? (
            ...
            <div className="max-w-2xl mx-auto mt-6 sm:mt-16 px-1">
              <div className="text-center mb-6 sm:mb-10">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 mb-4">
                  ...
                  <span className="text-xs font-semibold text-blue-600 uppercase tracking-wider">K.A. Design Accessories Ltd.</span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">
                {[
                  ...
                  <div key={f.title} className="p-4 rounded-xl bg-white border border-gray-200 text-center shadow-sm hover:shadow-md transition-shadow">
                    <span className="text-2xl">{f.icon}</span>
                    <p className="text-sm font-semibold text-gray-700 mt-2">{f.title}</p>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="space-y-1.5">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest px-1">Purchase Orders</p>
                  ...
                  {poNames.map((po, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold whitespace-nowrap transition-all duration-200 border
                          ${isActive
              ...
              {activePO && (
                <div className="flex items-center gap-3 px-1">
                  <p className="text-xs text-gray-500">
                    <span className="text-blue-600 font-semibold">{activePO}</span>
                    {" • "}
                    <span className="text-gray-400">{Object.keys(groupedData.po_groups[activePO]?.categories || {}).length} categories</span>
                  </p>
